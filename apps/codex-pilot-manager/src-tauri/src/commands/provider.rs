...
pub(crate) async fn import_official_snapshot_from_backup(
) -> Result<OfficialSnapshotImportResult, String> {
    ...
    .await
    .map_err(|error| format!("导入官方原版备份任务失败：{error}"))?
}

#[tauri::command]
pub(crate) async fn prepare_official_snapshot_after_clearing_relay(
) -> Result<OfficialSnapshotPrepareResult, String> {
    tauri::async_runtime::spawn_blocking(|| {
    ...
    .await
    .map_err(|error| format!("准备官方原版恢复点任务失败：{error}"))?
}

#[tauri::command]
pub(crate) async fn import_ccs_provider_profiles() -> Result<CcsImportResult, String> {
    tauri::async_runtime::spawn_blocking(|| {
        let mut state = load_provider_profiles();
    ...
    .await
    .map_err(|error| format!("导入 CCSwitch 配置任务失败：{error}"))?
}

#[tauri::command]
pub(crate) async fn apply_provider(request: ProviderApplyRequest) -> Result<String, String> {
    let profiles = load_provider_profiles();
...
pub(crate) async fn save_provider_profile(
    ...
    .await
    .map_err(|error| format!("保存中转配置档任务失败：{error}"))?
}
...
pub(crate) async fn activate_provider_profile(
    ...
    .await
    .map_err(|error| format!("启用中转配置档任务失败：{error}"))?
}
...
pub(crate) async fn delete_provider_profile(
    ...
    .await
    .map_err(|error| format!("删除中转配置档任务失败：{error}"))?
}

#[tauri::command]
pub(crate) async fn clear_provider() -> Result<String, String> {
    tauri::async_runtime::spawn_blocking(|| {
    ...
    .await
    .map_err(|error| format!("清除中转任务失败：{error}"))?
}
...
#[tauri::command]
pub(crate) async fn sync_provider_sessions(request: ProviderSyncRequest) -> Result<String, String> {
    tauri::async_runtime::spawn_blocking(move || {
        let target_provider = sanitize_provider_sync_target(
            request
                .target_provider
